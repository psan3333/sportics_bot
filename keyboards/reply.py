...
start_bot_kb = basic_reply_kb_builder(["/start", "/help"])

# two keyboards used for user registration
create_user_card_kb = basic_reply_kb_builder("Продолжить")
sex_kb = basic_reply_kb_builder(["Парень", "Девушка"], input_field_placeholder="Пол")

get_location_kb = ReplyKeyboardMarkup(
    keyboard=[[KeyboardButton(text="Разрешить", request_location=True)]],
...
sex_filters_kb = basic_reply_kb_builder(["Парни", "Девушки", "Неважно"], rows=[2, 1])
...
def main_bot_keyboard():
    ...
    """
    return basic_reply_kb_builder(
        main_kb_button_names, rows=[2, 2], input_field_placeholder="Действие"
    )
