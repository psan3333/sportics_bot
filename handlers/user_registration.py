...
async def wrong_user_form_name_input(message: Message, state: FSMContext):
    await message.answer(
        ...
        reply_markup=basic_reply_kb_builder(message.from_user.first_name),
    )
...
async def set_user_form_name(message: Message, state: FSMContext):
    ...
    await message.answer("Дальше введите ваш возраст.\nСколько вам полных лет?")
...
async def wrong_type_user_form_age_input(message: Message, state: FSMContext):
    await message.answer(
        "Пожалуйста, введите ваш возраст одним числом - количество ваших полных лет. ⌨️"
    )
...
@router.message(UserRegistrationForm.sex, F.text.in_(["Парень", "Девушка"]))
...
async def set_user_form_about(message: Message, state: FSMContext):
    ...
    example_photo = BufferedInputFile(
        example_photo_file.read(), "profile_photo_example.png"
    )
    await message.answer(
