...
async def start_users_filtereing(message: Message, state: FSMContext, _db: Database):
    ...
    profile_message_id = await UserProfile.show_user_profile(
        user_data, message=message, _db=_db, reply_markup=admin_check(users_to_check)
    )
...
async def switch_user_handler(
    ...
    if callback_data.action == "prev":
        ...
        await _db.delete_user(UsersContainer.users[user_idx]["user_id"])
        UsersContainer.users.pop(user_idx)

    if len(UsersContainer.users) > 0:
        ...
        await UserProfile.replace_profile_to_check(
            ...
            reply_markup=admin_check(users=UsersContainer.users, user_idx=user_idx),
        )
    else:
